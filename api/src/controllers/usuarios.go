...
func CriarUsuario(w http.ResponseWriter, r *http.Request) { //Criar um usuário
	...
	var usuario modelos.Usuario //Cria uma variável do tipo modelos.usuario para poder receber as informações do corpo da requisição

	if erro = json.Unmarshal(corpoRequest, &usuario); erro != nil { // Converte o corpo da requisição de JSON e joga dentro da struct usuario
		respostas.Erro(w, http.StatusBadRequest, erro)
		return
	}

	if erro = usuario.Preparar("cadastro"); erro != nil { //Realiza os tratamentos e verificação dos dados para serem colocados na struct
		respostas.Erro(w, http.StatusBadRequest, erro)
		return
	}
	...
	repositorio := repositorios.NovoRepositorioDeUsuarios(db) // passa a conexão o uma struct usuário (do repositórios)
	usuario.Id, erro = repositorio.Criar(usuario)             //chama o método criar da struct usuarios no repositórios passando como parâmetro a struct usuarios do tipo modelos.Usuario
	if erro != nil {
		respostas.Erro(w, http.StatusInternalServerError, erro)
		return
	}
...
func BuscarUsuarios(w http.ResponseWriter, r *http.Request) { //Busca todos os usuário que tenham um certo nome/nick
...
func BuscarUsuario(w http.ResponseWriter, r *http.Request) { //Busca um usuário com base no ID dele
...
func EditarUsuario(w http.ResponseWriter, r *http.Request) { //Edita (Atualiza) um usuário (Menos a senha!)
...
func DeletarUsuario(w http.ResponseWriter, r *http.Request) { //Deleta um usuário
